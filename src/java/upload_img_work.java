...
import LibraryAMuse.DBconnection;
import java.io.File;
 ...
 * @author Fede
 */
@WebServlet(urlPatterns = {"/upload_img_work"})
public class upload_img_work extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet upload_img_work</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet upload_img_work at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        } finally {            
            out.close();
...
if (name.equals("id_ex")){ id_ex=value;}
...
File uploadedFile = new File("/Users/Babol/Desktop/dVruhero/aMuseWebsite/build/web/imagesExhibition/" + fileName);

File uploadedFile2 = new File("/Users/Babol/Desktop/dVruhero/aMuseWebsite/web/imagesExhibition/" + fileName);

// Solo se veramente ho inviato qualcosa
if (item.getSize() > 0) {
    item.write(uploadedFile);
    item.write(uploadedFile2);
        DBconnection.EditImgWork(Integer.parseInt(id_work),fileName);    
